...
const getRecentActivity = item => item.coverage_status || ''
const getAccountablePerson = item => item.accountable_person || ''
const getCoverageAmount = item => formatMoney(item.coverage_amount)
const getPremium = item => formatMoney(item.annual_premium)
const getRiskCategoryName = item => item.risk_category?.name || ''
        ...
        <Datatable.Data>
          {#each items as item (item.id)}
              <Datatable.Data.Row on:click={() => redirect(`/items/${item.id}`)} clickable>
                ...
                <Datatable.Data.Row.Item>{getItemName(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>{getRecentActivity(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>{getAccountablePerson(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>{getCoverageAmount(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>{getPremium(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>{getRiskCategoryName(item)}</Datatable.Data.Row.Item>
                <Datatable.Data.Row.Item>
